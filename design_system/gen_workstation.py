...
<div class="wrap">
  ...
  <div class="bar">
    <button class="xbtn export" type="button">⬇ Exportar cambios (JSON)</button>
    <span class="savehint">Los cambios se guardan solos en este navegador.</span>
  </div>
...
(function(){
  ...
  document.querySelector('.xbtn.export').addEventListener('click',async function(){
    var out={month:'2026-09',exported_at:new Date().toISOString(),posts:[]};
    document.querySelectorAll('.post').forEach(function(post){
    ...
    var dl=await downloads();
    if(dl){try{await dl.save({filename:'idiem_cambios_sep2026.json',data:json});toast('Cambios exportados');return;}
      catch(e){if(e&&e.code==='declined'){toast('Exportación cancelada');return;}}}
    // fallback: mostrar en un cuadro para copiar
    var w=document.getElementById('lb');var pre=document.createElement('textarea');
    pre.value=json;pre.style.cssText='width:min(90vw,720px);height:60vh;font-family:monospace;font-size:12px';
    lbImg.style.display='none';w.querySelector('.lbbar').style.display='none';
    w.insertBefore(pre,w.querySelector('.close').nextSibling);w.classList.add('on');
  });
})();
</script>'''
